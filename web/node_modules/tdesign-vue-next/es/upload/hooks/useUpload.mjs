...
import { _ as _toArray } from '../../_chunks/dep-5f240748.mjs';
import { f as getCurrentDate, i as isOverSizeLimit, h as getFileList } from '../../_chunks/dep-652c99fe.mjs';
import { _ as _typeof } from '../../_chunks/dep-f038e2b4.mjs';
...
function useUpload(props) {
  ...
  var _useVModel = useVModel(files, modelValue, defaultFiles.value, props.onChange, "files"),
    ...
    setRawValue = _useVModel2[1];
  var uploadValue = computed(function () {
  ...
  var uploadFilePercent = function uploadFilePercent(params) {
    ...
    var operationUploadFiles = autoUpload.value ? toUploadFiles : rawValue;
  ...
  function uploadFiles(toFiles) {
    ...
    upload({
      ...
      if (status === "success") {
        ...
        setRawValue(_toConsumableArray(data.files), {
          trigger: "add",
          file: data.files[0]
        });
        xhrReq.value = [];
  ...
  function onInnerRemove(p) {
    ...
    if (props.isBatchUpload || !props.multiple) {
      ...
      var newVal = _toConsumableArray(uploadValue.value);
      newVal.splice(p.index, 1);
      setRawValue(newVal, changePrams);
    } else {
      if (p.index < uploadValue.value.length) {
        var _newVal = _toConsumableArray(uploadValue.value);
        _newVal.splice(p.index, 1);
        setRawValue(_newVal, changePrams);
      } else {
  ...
  var cancelUpload = function cancelUpload(context) {
    ...
    if (autoUpload.value) {
      ...
      setRawValue(uploadValue.value.map(function (item) {
