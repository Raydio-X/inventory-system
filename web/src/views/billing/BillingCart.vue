    ...
    <div v-if="cartItems.length > 0" class="cart-list">
      <div v-for="group in groupedCartItems" :key="group.productId" class="cart-group">
        ...
        <div class="group-header">
          ...
          <!-- 价格参考 -->
          <div v-if="currentCustomer && currentCustomer.id !== 'customer-walk-in'" class="price-reference">
            <div class="ref-item">
...
const getCustomerHistoryPrice = (productId) => {
  if (!currentCustomer.value || !productId || currentCustomer.value.id === 'customer-walk-in') return null
  return customerHistoryPrices.value[`${currentCustomer.value.id}_${productId}`] || null
}

// 加载客户历史购买价格
const loadCustomerHistoryPrices = async () => {
  if (!currentCustomer.value || currentCustomer.value.id === 'customer-walk-in' || cartItems.value.length === 0) {
  ...
  try {
    ...
    if (res) {
      ...
      Object.entries(res).forEach(([productId, price]) => {
        priceMap[`${currentCustomer.value.id}_${productId}`] = price
      })
